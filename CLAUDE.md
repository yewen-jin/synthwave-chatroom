...
Runtimes without an `EnterWorktree` tool get the same layout with plain git: `git worktree add .claude/worktrees/<name> -b <name>`.
...
_Update at the end of each session: what's done, what's next, any blocker. Open tasks and decisions live in `__context__/to-do.md` — keep entries here to session summaries and point there._

- 18 Jul 2026 (later) — **Orchestration workflow refinements** (docs only, no code): `WORKLOG.md` gets the built-in `merge=union` driver in `.gitattributes`, so parallel worktree appends to the log no longer conflict; `__context__/to-do.md` rebuilt as the single open-items list (real-device gate now a checklist, open decisions + tier-3 review items consolidated, stale Feb 2026 items marked unverified); runtime-neutral equivalents documented — `git worktree add .claude/worktrees/<name> -b <name>` for runtimes without `EnterWorktree`, and the notes-keeper dispatch as a paste-the-agent-file subagent brief (see Decision Log in `AGENTS.md`).
