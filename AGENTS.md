...
- **One room per socket.** The "joined the wrong room?" recovery joins a second room from the same socket; the server leaves and cleans the previous room first (same path as `disconnect` — `leaveCurrentRoom()`), so a socket never receives two rooms' broadcasts and a switched-away username never ghosts a slot. The switch happens only after the new room accepts the join, so a room-full/name-taken rejection leaves the player in their current room.
- Join/check payloads are validated server-side (`parseJoinPayload()`) before any destructuring — Socket.IO handlers have no exception boundary, so a crafted `null` payload used to be a process-crash vector for the whole server, narrative show included. Usernames are length-capped (20, matching `room.html`) but not charset-restricted; all rendering is `textContent`.
...
- **Open items live in `__context__/to-do.md`** — the single list of outstanding tasks and decisions (the real-device gate is a checklist there). Update it in the same session as the work; the notes-keeper does not write it (its surface stays `WORKLOG.md`-only), and `WORKLOG.md` entries keep their "Still open" tails as the historical record.
